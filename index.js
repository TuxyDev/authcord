...
function url(client, urlRedirect, scopes = ['identify']) {
    if (!urlRedirect) 
        ...
        return `https://discordapp.com/oauth2/authorize?client_id=${client.id}&scope=${scopes.join("%20")}&response_type=code&redirect_uri=${redirect}`;
}
 ...
 * 
 * @returns {JSON} the json sent directly from discord.
 */
async function callback(client = {}, code, type = "authorization_code") {
 ...
 * @param {String} refreshToken the fallback refresh token.
 * 
 * @returns {JSON} the user's guild data.
 */
 ...
 * @param {String} refreshToken the fallback refresh token.
 * 
 * @returns {Boolean} if the user joined the channel successfully.
 */
...
module.exports = {exchangeUser, exchangeGuilds, joinGuild, url, callback, redirect};